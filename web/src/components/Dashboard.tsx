...
export default function Dashboard({ filename, report }: DashboardProps) {
  ...
  const calculateMetrics = () => {
    ...
    report.profiles.forEach(profile => {
      const tonnage = profile.total_weight / 1000 // Convert kg to tonnes
    ...
    report.plates.forEach(plate => {
      const tonnage = plate.total_weight / 1000 // Convert kg to tonnes
      totalTonnage += tonnage
      platesTonnage += tonnage
    })

    // Get bolt count from report (fastener_count is now provided by backend)
    boltCount = report.fastener_count || 0

    // Count assemblies - from report.assemblies array
    if (report.assemblies && Array.isArray(report.assemblies)) {
      assemblyCount = report.assemblies.length
    }

    // Count single parts - sum of piece_count from profiles and plates
